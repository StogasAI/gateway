...
| `stogas-linux-6.18` | Minimal Linux 6.18.35 guest kernel with built-in EFI, virtio, configfs/TSM, SEV-SNP report paths, QEMU `fw_cfg` sysfs, and explicit hardening options. | Guix `linux-libre@6.18.35` from the pinned Guix channel. | Kernel config hash is recorded in each manifest; required built-ins are listed in `stogas/release/pins.lock.json`. Raw packet sockets and legacy virtio PCI are explicitly disabled. |
